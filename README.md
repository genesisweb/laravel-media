...
[![Packagist Version](https://img.shields.io/packagist/v/genesisweb/media.svg)](https://packagist.org/packages/genesisweb/media)
...
You should use the `GenesisWeb\Media\MediaUploader` class to handle file uploads.
...
In order to associate a media item with a model, you must first include the `GenesisWeb\Media\HasMedia` trait.
...
use GenesisWeb\Media\Facades\Conversion;
